...
void actualiser_site_web(int etatRobot, float vitD, float vitG, float posX, float posY, float CAPTEUR_CG, float CAPTEUR_AG, 
                        float CAPTEUR_AD, float CAPTEUR_CD, bool etatGyro, float dist, float dureeMission, float dureeTotal) {
  // Exemple de trame : $VIT#12.3$POSX#45.6$POSY#0.0\n
  char buf[64];
  int len = snprintf(buf, sizeof(buf),
                     "$ETATROBOT#%.2f$VITD#%.2f$VITG#%.2f$POSX#%.2f$POSY#%.2f$CAPTEUR_CG#%.2f$CAPTEUR_AG#
                     %.2f$CAPTEUR_AD#%.2f$CAPTEUR_CD#%.2f$ETATGYRO#%.2f$DIST#%.2f$DUREEMISSION#%.2f$DUREETOTAL#%.2f\n",
                     vit, posX, posY);
